...
every trip record includes a cryptographic signature embedded in the trip ID. This signature proves that:
...
1. TRIP ID FORMAT DETECTION (verify.html lines 870-895):
   ...
   - Legacy format (4 parts): Data before November 8, 2025 without signatures (expect ~20)
   - November 8, 2025 and later format (5 parts): deviceID-dest-date-time-signature (with HMAC)
...
5. CATEGORIZATION (verify.html lines 975-990):
   - authenticated: November 8, 2025 and later trips with valid signatures
   - legacy: Trips before November 8, 2025 without signatures (expected ~20)
...
2. Legacy Format (Before November 8, 2025):
...
3. November 8, 2025 and Later Format (With HMAC):
...
Expected Results:
- authenticated: Most trips (November 8, 2025 and later)
- legacy: ~20 trips (before November 8, 2025)
...
Example workflow:
  ...
  3. Review results:
     ...
     - Confirm all November 8, 2025 and later trips are authenticated
...
def compute_signature_from_trip_id(trip_id: str, secret_key: str) -> str:
    ...
    - Trip IDs from November 8, 2025 and later have format: deviceID-dest-date-time-signature
...
def verify_trip_signature(trip_id: str, secret_key: str) -> Dict:
    ...
    1. Detects trip ID format (UUID, legacy 4-part, or November 8, 2025 and later 5-part)
    2. For November 8, 2025 and later format: extracts baseTripId and provided signature
    ...
    CATEGORIES:
    - legacy: Trips before November 8, 2025 or UUID format (no signature validation)
    - authenticated: November 8, 2025 and later trips with valid signatures
    - invalid: November 8, 2025 and later trips with signature mismatches
    - error: Malformed trip IDs or missing trip IDs
    ...
    # Legacy format (4 parts): Data before November 8, 2025 without signatures
